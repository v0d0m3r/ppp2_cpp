...
Ввод осуществляется из потока типа istream; вывод - в поток ostream.
...
Ввод из потока istream через Token_stream с именем ts.
*/
...
public: 
    Token_stream();         // Создает объект класса Token_stream,
                            // считывающий данные из потока ввода
