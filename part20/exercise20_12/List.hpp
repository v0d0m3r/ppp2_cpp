...
List<Elem, A>::List(int s) : List_base<Elem, A>{A(), s}
{
    ...
    uninitialized_copy(&def, &def + 1, this->first.get());
    uninitialized_copy(&def, &def + 1, this->last.get());
...
template<typename Elem, typename A>
List<Elem, A>::List(initializer_list<Elem> lst)
    : List_base<Elem, A>{A(), static_cast<int>(lst.size())}
{
