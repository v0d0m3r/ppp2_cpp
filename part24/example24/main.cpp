...
void task24_1()
{
    cout << "Количетсво байтов в типе char:\t\t"
         << sizeof(char) << '\n'
         << "Количетсво байтов в типе short:\t\t"
         << sizeof(short) << '\n'
         << "Количетсво байтов в типе int:\t\t"
         << sizeof(int) << '\n'
         << "Количетсво байтов в типе long:\t\t"
         << sizeof(long) << '\n'
         << "Количетсво байтов в типе float:\t\t"
         << sizeof(float) << '\n'
         << "Количетсво байтов в типе double:\t"
         << sizeof(double) << '\n'
         << "Количетсво байтов в типе int*:\t\t"
         << sizeof(int*) << '\n'
         << "Количетсво байтов в типе double*:\t"
...
void task24_2_3()
{
    ...
    cout << "Количетсво байтов в объекте a:\t\t"
         << sizeof(a) << '\n'
         << "Количетсво байтов в объекте b:\t\t"
         << sizeof(b) << '\n'
         << "Количетсво байтов в объекте c:\t\t"
         << sizeof(c) << '\n'
         << "Количетсво байтов в объекте d:\t\t"
         << sizeof(d) << '\n'
         << "Количетсво байтов в объекте e:\t\t"
         << sizeof(e) << '\n'
         << "Количество элементов в а:\t\t"
         << a.size() << '\n'
         << "Количество элементов в b:\t\t"
         << b.size() << '\n'
         << "Количество элементов в c:\t\t"
         << c.size() << '\n'
         << "Количество элементов в d:\t\t"
         << d.size() << '\n'
         << "Количество элементов в e:\t\t"
         << e.size() << '\n';
}
...
public:
    Sum_cmplx() {}
    void operator()(cmplx zz) { z += zz; }
    cmplx sum() const { return z; }
};
...
void task24_6()
{
    ...
    for (Index i=0; i < m; ++i) b(i) = i;

    Matrix_m mat(n, m);
    for (Index i=0; i < mat.dim1(); ++i)
        for (Index j=0; j < mat.dim2(); ++j)
            mat(i, j) = a(i) * b(j);
    cout << mat << '\n';
}
...
void task24_7()
{
    Matrix<cmplx> m(10);
    Index i=0;
    for (cmplx z; i<m.size() && cin>>z; ++i) m(i) = z;
    Sum_cmplx sz;
    for (Index i=0; i < m.dim1(); ++i) sz(m(i));
    cout << "Сумма чисел: " << sz.sum() << '\n';
}
...
void task24_8()
{
    ...
    for (Index i=0; i<m.dim1(); ++i)
        for (Index j=0; j<m.dim2() && cin>>d; ++j)
            m(i, j) = d;
    cout << m << '\n';
}
