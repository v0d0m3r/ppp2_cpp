...
Версия 15, август 2017 год

Эта программа реализует основные выражения калькулятора.

История изменений:
    09.03.16 Первая глобальная версия
    15.08.17 Variable became as Pair
...
public:    
    ...
    Symbol_table() {}
private:
    vector<Tuple<string, int, bool>> var_table;         // Хранилище переменных
};
...
void Symbol_table::set_value(string s, double d)
{
    for (auto& v : var_table)
        if (v.first==s && !v.third) {
            v.second = d;
            return;
        }
        else if (v.first==s && v.third) {
...
double Symbol_table::define_name(string var, double val, bool is_constant)
{
    ...
    var_table.push_back(Tuple<string, int, bool>(var, val, is_constant));
    return val;
}
